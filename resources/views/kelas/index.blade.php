...
@php
  ...
  $makeKelasCard = function ($kelasItem) {
    $kegiatan = $kelasItem->kegiatan ?? null;

    if (!$kegiatan) {
      return null;
    }

    $jenisRaw = strtolower($kegiatan->jenis_kegiatan ?? 'kegiatan');
    $modaRaw = strtolower($kegiatan->moda ?? '-');
    $jenisPelatihanRaw = strtolower($kegiatan->jenis_pelatihan ?? '');

    $waktuText = '-';

    if (!empty($kegiatan->waktu_pelaksanaan)) {
      $waktuText = \Carbon\Carbon::parse($kegiatan->waktu_pelaksanaan)->format('d M Y, H:i');
    }
    ...
    $moodleUrl = $kegiatan->moodle_course_url ?? '';
    ...
    if ($moodleUrl) {
      if ($isTerbimbing) {
        $moodleAktif = !empty($kelasItem->moodle_injected_at);
      } else {
        $moodleAktif = true;
      }
    }
    ...
    return [
      ...
      'nama_peserta' => $kelasItem->nama ?? auth()->user()->name ?? '-',
      'instansi' => $kelasItem->asal_instansi ?? $kelasItem->instansi ?? '-',
    ...
    <div class="kelas-page-card">
      ...
      <div class="kelas-toolbar">
        ...
        <div class="kelas-filter-buttons">
          <button type="button" class="kelas-filter-btn is-active" data-jenis="">
            Semua
          </button>

          <button type="button" class="kelas-filter-btn" data-jenis="konsultasi">
            Konsultasi
          </button>

          <button type="button" class="kelas-filter-btn" data-jenis="webinar">
            Webinar
          </button>

          <button type="button" class="kelas-filter-btn" data-jenis="pelatihan">
            Pelatihan
          </button>
        </div>

        <button type="button" id="resetKelasFilter" class="kelas-reset-btn">
          Reset
        </button>
      </div>
      ...
      <section class="kelas-section">
        <div class="kelas-section-heading">
          ...
          <p>
            Kegiatan yang belum melewati waktu pelaksanaan.
          </p>
        </div>
              ...
              >
                ...
                <div class="kelas-card-body">
                  <div class="kelas-badges">
                    ...
                    <span class="{{ $card['moodle_status_class'] }}">
                      {{ $card['moodle_status'] }}
                    </span>
                  </div>
                  ...
                  <div class="kelas-actions">
                    ...
                    @if($card['moodle_aktif'] && $card['moodle_url'])
                      <a
                        href="{{ $card['moodle_url'] }}"
                        target="_blank"
                        rel="noopener"
                        class="kelas-main-btn"
                      >
                        Masuk Course Moodle
                      </a>
      ...
      <section class="kelas-section riwayat-section">
        <div class="kelas-section-heading">
          ...
          <p>
            Kegiatan yang waktu pelaksanaannya sudah lewat akan tersimpan di sini.
          </p>
        </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function filterKelas() {
      ...
      if (aktifEmpty) {
        aktifEmpty.hidden = aktifVisible > 0 || countSectionCards('aktif') === 0;
      }

      if (riwayatEmpty) {
        riwayatEmpty.hidden = riwayatVisible > 0 || countSectionCards('riwayat') === 0;
      }
    }
